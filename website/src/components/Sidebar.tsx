...
export default function Sidebar() {
	...
	useEffect(() => {
		...
		manifest?.categorizedDocs?.forEach((cat) => {
			initialOpen[cat.categoryGeneratedIndex.slug.replace("/category/", "")] =
		...
		>
			<nav className="flex-1 flex overflow-y-auto p-6 flex-col gap-6">
				...
				{manifest?.categorizedDocs?.map((cat) => {
					const key = cat.categoryGeneratedIndex.slug.replace("/category/", "");
						...
						<div key={key} className="flex flex-col gap-3">
							<header className="flex items-start justify-between gap-2">
								<Link
									...
									params={{
										category: cat.categoryGeneratedIndex.slug.replace(
											"/category/",
											"",
										),
									}}
									...
									title={cat.categoryGeneratedIndex.description || undefined}
								>
									{cat.label}
								</Link>
									...
									{cat.files.map((file) => (
										<li key={file.id}>
											<Link
												...
												params={{
													category: key,
													doc: removeFileExtension(file.id.split("/").pop()!),
												}}
												className={clsx(
