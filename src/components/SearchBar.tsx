...
const SearchBar = forwardRef<HTMLInputElement, SearchBarProps>(({ isOpen, onClose }, ref) => {
  ...
  const normalizedQuery = useMemo(() => query.toLowerCase().replace(/[-_]/g, ' ').trim(), [query]);
  const searchTerms = useMemo(() => normalizedQuery.split(/\s+/).filter(t => t.length > 0), [normalizedQuery]);
    ...
    categories.forEach((cat) => {
      const matches = cat.files
        .map((file): SearchResult | null => {
          ...
          const allTermsMatched = searchTerms.every(term => normalizedFileName.includes(term));
